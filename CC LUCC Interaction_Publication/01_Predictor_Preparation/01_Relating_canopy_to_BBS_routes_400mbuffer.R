##################################################################################################
########                                                                                  ########
########              Calculating proportional canopy cover for each BBS route            ######## 
########                              from the iGFC data                                  ########
##################################################################################################

# Author: Kimberly Thompson

# This code calculates the proportion of canopy/forest cover within each BBS route
# from the period of 1992 - 2018 using the global tree-canopy dataset (created by Ruben). 

# In each raster, canopy cover is represented as percent coverage.

# ****NOTE: Performed on HPC Cluster

# Produces datasets:
# 'Canopy_400mbuff_BBS_1.csv'
# and on through _27.csv, which each number corresponding to yrs 1992-2018
...
lulc.predictors <- data.frame(Year = rep(seq(from = 1992, to = 2018, by = 1),
                              ...
                              Max = integer(length(bbs.routes$uniq_rt) *
                                              length(seq(from = 1992, to = 2018, by = 1))),
                              CV = integer(length(bbs.routes$uniq_rt) *
                                             length(seq(from = 1992, to = 2018, by = 1))))


# Fill predictor columns with NA values
lulc.predictors[ , c(3:7)] <- NA
...
lulc.path <- "/gpfs1/data/idiv_meyer/01_projects/Kim_T/Analysis_Output/Canopy_Cover"
...
for (j in 1:length(unique(lulc.predictors$unique_route))) { # number of routes
  ...
  if(bbs.routes$Country[j] == "US" & bbs.routes$St_Abrv[j] != "AK") {
    ...
    tmp.df_sum$CV <- sd(tmp.df$Cover, na.rm = TRUE) / mean(tmp.df$Cover, na.rm = TRUE) * 100
    
    
    # Fill in the predictor dataframe with the metric values
    lulc.predictors[lulc.predictors$Year == year & 
                      lulc.predictors$unique_route == unique(lulc.predictors$unique_route)[j], c(3:7)] <-
      tmp.df_sum[1, ]
...
# Save the file
setwd("/gpfs1/data/idiv_meyer/01_projects/Kim_T/Analysis_Output/Canopy_Cover_BBS")

write.csv(lulc.predictors, file = paste("Canopy_400mbuff_BBS_", i, ".csv", sep = ""), row.names = FALSE)
